...
class RsgeJobs
  def initialize
    if !defined?(@@jobs)
      if $rspec_init == true
        ...
        doc = Nokogiri::XML(open("|qstat -r -xml -u '*' -s a"))
      end
  ...
  def each_when_name_is(name)
    @@jobs.each { |job| yield RsgeJob.new :jobid => job[:jobid] if job[:job_name] == name }
  end

  def each_when_state_is(state)
    @@jobs.each { |job| yield RsgeJob.new :jobid => job[:jobid] if job[:state] == state }
  end

  def each_when_owner_is(owner)
    @@jobs.each { |job| yield RsgeJob.new :jobid => job[:jobid] if job[:owner] == owner }
  end
